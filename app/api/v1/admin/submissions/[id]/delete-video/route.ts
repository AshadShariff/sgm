...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!video) {
      return NextResponse.json({ error: "Video not found in submission" }, { status: 404 })
    }
    ...
    if (!publicId) {
      return NextResponse.json({ error: "Video public_id not found" }, { status: 400 })
    }
    ...
    submission.videos = submission.videos.filter((v: any) => v.url !== videoUrl)
    ...
    console.log("Video removed from submission. Remaining videos:", submission.videos.length)
